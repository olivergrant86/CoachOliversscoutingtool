...
// disambiguation: optional { "K Garrett": [{ slot: 3, name: "Kyle Garrett" }, { slot: 7, name: "Kevin Garrett" }] }
// lineupSize: how many spots the batting order cycles through before repeating (9 is standard).
// Both are for resolving two same-name players on one team by tracking whose turn it
// actually is in the order — see glogResolveBatterName below for how it's applied.
function parseGameLogText(text, disambiguation, lineupSize) {
  disambiguation = disambiguation || {};
  lineupSize = lineupSize || 9;
  const paCountBySide = { home: 0, visitor: 0 };

  // Increments this side's plate-appearance count and returns the name to actually use
  // for this batter — the raw short name, unless it's a name we've been told is shared
  // by two players on this side, in which case the batting-order slot this PA falls on
  // (cycling through lineupSize spots) picks which specific player it really is.
  function glogResolveBatterName(rawName, side) {
    if (side === "home" || side === "visitor") paCountBySide[side]++;
    const options = disambiguation[rawName];
    if (!options || !options.length || (side !== "home" && side !== "visitor")) return rawName;
    const slot = ((paCountBySide[side] - 1) % lineupSize) + 1;
    const match = options.find(o => o.slot === slot);
    return match ? match.name : rawName; // no rule for this exact slot — fall back to the raw name rather than guessing
  }
  ...
  blocks.forEach(block => {
    ...
    const rawBatter = batterMatch ? batterMatch[1] : null;
    if (!rawBatter) { errors.push(`Couldn't read batter name from: "${narrative}"`); return; }
    const batter = glogResolveBatterName(rawBatter, block.side);
